...
pub(super) async fn api_dashboard_summary(
    ...
    let result = (|| -> Result<Value> {
        ...
        let recent_requests = query_logs(
            db_path,
            &LogQuery {
                limit: 5,
                offset: 0,
        ...
        // error and warn pages and keep the 10 most recent entries.
        let mut recent_errors = list_events(db_path, 10, None, Some("error"), None)?.events;
        recent_errors.extend(list_events(db_path, 10, None, Some("warn"), None)?.events);
